...
This directory contains Architecture Decision Records (ADRs) for Open Brain, using the [MADR](https://adr.github.io/madr/) (Markdown Architectural Decision Records) format.
...
Product behavior — what a tool, flow, or client should do — belongs in
[`docs/specs/`](../specs/SPECS.md), not here.
...
Accepted ADRs are append-only. The only edit to an accepted record is a status
flip. To change a past decision, write a new ADR that supersedes it.
...
Use the project skills in `.agents/skills/` to keep records at decision altitude:

- `adr-author` — draft, capture, or supersede an ADR
- `adr-review` — review a record without editing it
