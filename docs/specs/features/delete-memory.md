...
- [ ] `S1` The success `memory` includes `id`, `content`, `source`, `metadata`, `created_at`, `updated_at`, plus `embedding_model` / `embedded_at` when present
- [ ] `S1` A valid UUID that does not exist is **404** `{ error: "Memory not found." }`
- [ ] `S1` A non-UUID `id` is **400** with `` `id` must be a valid UUID. ``; nothing is deleted
...
| -------- | ----------------- |
| Unknown UUID | 404; store unchanged |
