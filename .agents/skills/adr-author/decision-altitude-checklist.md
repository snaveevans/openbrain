...
- **Mechanism (push down):** table schemas, function/module names, step-by-step algorithms, wire formats, concrete API or MCP tool shapes, retry counts, config values.
...
> Example: an ADR may decide that hosted MCP runs on Cloudflare Workers (decision). The worker routes, wrangler bindings, and OAuth metadata URLs (mechanism) live in a feature spec, not the ADR.
