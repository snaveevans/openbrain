---
name: adr-author
description: Author, capture, or supersede an Architecture Decision Record (ADR). Keeps ADRs at decision altitude — the choice, its drivers, the options weighed, and the consequences — and pushes implementation mechanism down into feature specs. Treats the ADR collection as an append-only ledger. Use whenever Open Brain needs to record a hard-to-reverse technical choice.
---
...
**Run this before drafting. Do not infer the ledger's contents from memory or context:**

```bash
ls docs/decisions/[0-9][0-9][0-9][0-9]-*.md 2>/dev/null | sort
```

The next number is the highest one that command returns, + 1, zero-padded to four
digits. Numbers are assigned sequentially and never reused.

A guessed number silently collides with an existing record and corrupts the ledger
this skill exists to protect. If the command returned nothing, confirm the directory
exists before assuming the ledger is empty.
...
**3. Drivers** — Name the qualities, constraints, and concerns that shape the choice (e.g. reversibility, operational cost, one-person ops, consistency with existing MCP clients). These are what the options get judged against.
